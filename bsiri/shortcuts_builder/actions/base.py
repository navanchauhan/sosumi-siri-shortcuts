...
class RawAction(BaseAction):
    """Passthrough action for unrecognized action types.

    Preserves the raw plist dict so it survives round-trips without data loss.
    """
    keyword = 'raw_action'
    ...
    def dump(self):
        return self.data.get('_raw_plist', {
            'WFWorkflowActionIdentifier': self.data.get('identifier', 'unknown'),
            'WFWorkflowActionParameters': self.data.get('parameters', {}),
        })
